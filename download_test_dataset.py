"""
Utility script to extract the compressed BDD100K YOLO test dataset.

This script:
1. Checks if bdd100k_yolo_test_set.zip exists in bdd100k_yolo_tuning_zipped/
2. If not found, downloads it from Google Drive using gdown
3. Extracts only the test split for quick experiments without needing to process the full 100K dataset
...
def process_limited_dataset():
    """Extract the compressed test dataset."""
    ...
    if not compressed_file.exists():
        ...
        if not download_from_gdrive(GDRIVE_FILE_ID, compressed_file):
            ...
            print("You can generate it by running: python download_test_dataset.py")
            return
    ...
    if extract_dir.exists() and (extract_dir / 'data.yaml').exists():
        ...
        # Count extracted files (test split only)
        test_images = extract_dir / 'images' / 'test'
        test_labels = extract_dir / 'labels' / 'test'
        
        total_images = sum(len(list((extract_dir / d).glob('*.jpg'))) 
                          for d in test_images if (extract_dir / d).exists())
        total_labels = sum(len(list((extract_dir / d).glob('*.txt'))) 
                          for d in test_labels if (extract_dir / d).exists())
        
        print(f"  Test Images: {total_images:,}")
        print(f"  Test Labels: {total_labels:,}")
        print(f"\nYou can now use this test dataset for validation experiments!")
    else:
